...
The **Model Context Protocol** (MCP) is an open standard, [released by Anthropic in late 2024](https://www.anthropic.com/news/model-context-protocol), for how AI assistants connect to the systems where your business actually runs — CRM, data warehouse, ticketing, internal documents. A competent platform team would already factor a shared Salesforce or warehouse client out of individual feature work; MCP is not what *makes* that reuse possible. What MCP adds is a **standard contract** for that shared layer: one server, consumable by any MCP-capable AI client, portable across vendors. The protocol standardises how AI tools *connect* to systems — not model behaviour, not safety, not orchestration, and not the underlying platform engineering discipline. Those remain the genuinely hard parts. Everything below is implications.
...
*For the first feature or two, MCP costs more — you pay upfront for shared infrastructure (server, auth, observability, governance) that one feature alone doesn't need. From around the third feature, the cost of each new feature drops sharply because the integration is already in place. The honest caveat: a disciplined platform team already produces a similar curve through ordinary shared-service work. MCP's contribution is mostly that the shared layer has a standard contract, so the same server is consumable by every AI client — yours and your vendors' — without re-wrapping.*

The strawman version of this chart compares MCP to a world where three teams each write their own Salesforce client. That world exists, but it's a description of weak platform engineering, not of life-before-MCP. A mature org would already centralise that integration regardless of AI. The fair comparison is: *given* you'd build a shared Salesforce service either way, what does MCP change?

Two things, mainly. First, the wrapper between your shared service and each AI client stops being bespoke — one MCP server is consumable by Claude, by a code-editor copilot, by an internal agent, by whatever AI tool ships next quarter, without per-client glue. Second, the contract is the same one your vendors are building against, so reference servers for common SaaS systems (Slack, GitHub, Linear, etc.) are available off the shelf rather than as in-house work.

Where MCP genuinely *doesn't* help: it doesn't write your tool descriptions, design your tool surface, or replace the platform-engineering discipline of deciding what to share and what to keep local. If your org doesn't currently consolidate integrations behind shared services, adopting MCP won't fix that — it will just give the existing fragmentation a new file format.

For a CFO: the unit-economics shift is real but smaller than the marketing suggests. The bend in the curve mostly accrues to orgs that *also* invest in platform engineering. The portion that's specifically attributable to MCP is the part you'd otherwise spend re-wrapping the same shared service for each new AI client and vendor. That portion grows as the number of AI tools in the company grows — which, for most orgs, is the direction of travel.
...
In commercial terms, Marlin's situation before MCP: three AI features in flight, each by a different team, each writing its own Salesforce wrapper for whichever AI client it targets. Two of the three have inconsistent rate-limit handling and different audit-log shapes. Their security team can't easily answer "what data can each AI feature reach?" because the wiring is everywhere. A more disciplined Marlin would have consolidated that wiring behind a shared Salesforce service regardless of MCP — and saved most of the duplication. The bit that's harder to fix without a standard is the per-AI-client wrapper around that service.

After MCP: one Salesforce MCP server, owned by the data platform team, exposing the shared service in a contract every MCP-capable AI client can consume. The fourth, fifth, sixth feature each reuse it — whether they're built on Claude, an internal agent, or a copilot embedded in the IDE. The security team has *one* place to ask "what does this server expose, and to whom?" The vendor-negotiation story for their model provider improves: the integration layer is portable across assistants. The platform-engineering work (auth, observability, scoping) is the same work they would have done anyway; the standardised contract is the part MCP specifically buys them.
