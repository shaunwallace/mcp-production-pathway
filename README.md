...
- **Week 12 — Security, MCP-specific threats, PII, close-out.** Full STRIDE threat model committed at `decisions/threat-model.md`. Defences for the MCP-specific attacks not covered by OWASP: tool poisoning (registry allow-list), indirect prompt injection via tool outputs (envelope + scanner + audit event), confused-deputy via sampling (system-prompt allow-list + per-tenant sampling budget), schema confusion (zod-derived schemas, contract-tested), resource-link spoofing (URI scheme allow-list). Adversarial eval set with 12+ cases. Data-retention policy with data classes, retention windows, and named gaps. Weekly osv-scanner + CycloneDX SBOM in CI. Third and final memo (`03-security-posture.md` — what I got right, what I got wrong, posture today, what I'd do differently). Short closing callout on adjacent topics (agent frameworks, memory, A2A) deliberately out of scope.
